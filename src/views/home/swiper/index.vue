<template>
  <div>
    <div style="margin-bottom: 16px">
      ...
      <a-modal v-model:visible="formvisible" :footer="null" title="添加轮播图">
        <a-form :model="formState" v-bind="layout" name="nest-messages" :validate-messages="validateMessages"
          @finish="onFinish">
          <a-form-item :name="['fileList']" label="图片" :rules="[{ required: true, message: '图片不能为空!' }]">
          ...
          <a-form-item :name="['user', 'sort_num']" label="排序值" :rules="[
    ...
    }" :columns="columns" :data-source="tableData.data" :pagination="pagination" :row-key="(record) => record.id"
      @change="changePage">
