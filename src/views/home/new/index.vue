    ...
    <div style="margin-bottom: 16px">
      ...
      <a-modal v-model:visible="formvisible" :footer="null" title="添加商品" @cancel="flushForm">
        <a-form :model="formState" v-bind="layout" name="nest-messages"
          :validate-messages="{ required: '${label}不能为空' }" @finish="onFinish" ref="editUserFormRef">
          <a-form-item :name="['user', 'goods_name']" label="商品名称" :rules="rule">
            <a-input v-model:value="formState.user.goods_name" />
          </a-form-item>
          <a-form-item :name="['user', 'goods_id']" label="商品编号" v-if="isWrite">
  ...
  setup() {
    ...
    const handleOk = () => {
      ...
      if (state.selectedRowKeys.length !== 0) {
        ...
        delNewGoods({ goods_ids: state.selectedRowKey })
          .then(function (res) {
            currentPage()
          })
      }
    ...
    function updateOther(current) {
      findNewGoods({ page: current, size: 5 })
        .then(function (res) {
          data.arr = res.data.kind_goods
          pagination.total = res.data.size
        })
