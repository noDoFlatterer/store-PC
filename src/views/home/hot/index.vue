<template>
  <div>
    <div style="margin-bottom: 16px">
      ...
      <!-- 添加的表单 -->
      <a-modal v-model:visible="formvisible" :footer="null" title="添加轮播图" @cancel="cancel">
        <a-form :model="formState" v-bind="layout" name="nest-messages" :validate-messages="validateMessages"
          @finish="onFinish" ref="formRef">
          <a-form-item :name="['user', 'goods_name']" label="商品名称" :rules="[{ required: true }]">
            <a-input v-model:value="formState.user.goods_name" />
          </a-form-item>

          <a-form-item :name="['user', 'goods_id']" label="商品编号" :rules="[{ required: true }]">
            <a-input-number class="goodsid" v-model:value="formState.user.goods_id" />
          </a-form-item>
          <a-form-item :name="['user', 'sort_num']" label="排序值" :rules="[{ required: true }]">
    ...
    <a-table :row-selection="rowSelection" :columns="columns" :data-source="data.nowdata" :pagination="pagination"
      :row-key="(record) => record.goods_id" @change="changePage">
